...
def main(workflow_id):
    ...
    try:
        ...
        logger.error(f"workflow_id are always UUIDs and the workflow_id '{workflow_id}' is not.")
        sys.exit(1)
    ...
    try:
        ...
        for entity in entities:
            if not pg_instance.table_exists(schema_entities[entity]['table_name']):
                raise Exception(f"Required table '{schema_entities[entity]['table_name']}' does not exist. Please create it first.")
        

        s3_file_path = metadata_instance.get_ingestor_output_file_path(workflow_id=workflow_id)
        ...
        if s3_file_path is None:
            ...
            for entity in entities:
                try:
                    ...
                    df_normalized = df_columns_normalization(dataframe=df, column_schema=schema_entities[entity]['schema'])
                    
                    pg_instance.insert_dataframe(dataframe=df_normalized, table_name=table_name)

                    execution_metadata[entity]['records_inserted'] = len(df_normalized)
                
                except Exception as e:
                    execution_metadata[entity]['traceback'] = traceback.format_exc()
                    logger.error(f'Error processing/loading data to table {table_name}: {e}')
                
                finally:
                    metadata_instance.insert_metadata(code_step='handler', metadata=execution_metadata, entity=entity)


    
    except Exception as e:
        execution_metadata['traceback'] = traceback.format_exc()
        metadata_instance.insert_metadata(code_step='handler', metadata=execution_metadata)
        raise e
...
#     main("d8faa293-bd26-41ba-8d46-cc16bf31f975")