...
def main(workflow_id):
    ...
    try:
        output_filename = f"{execution_id}_{current_datetime.strftime('%Y%m%dT%H%M%SZ')}.json"
        logger.info(f"Execution started. UUID: {execution_id}, Timestamp: {current_datetime.isoformat()}")


        last_fetch_date = s3_instance.get_last_successfull_fetch_date(code_step='ingestor')
        ...
        jsons, number_of_files_fetched = s3_anon_instance.get_hour_files_from_bucket(s3_bucket, code_fetch_date)
        if not jsons:
        ...
        execution_metadata['file_destination_path'] = f"s3://{s3_data_bucket}/{output_filename}"
